...
class CheckoutController extends Controller
{
    ...
    public function pay_credit_safrapay(Request $request) {
        // Salva nos cookies meio de pagamento escolhido em caso de redirect back
        session()->flash('pay', 'credit_card_safrapay');
        ...
        // Invés de disparar o job que realiza a conexão com o endpoint
        // já realizar a tarefa
        // Fazer uma função privada aqui para executar
    ...
    public function pay_credit_card_pagarme(Request $request)
    {
        ...
        if ($order->coupon) {
            $calc_coupon = CouponHelper::instance()->get_amount($order->coupon, json_decode($order->products))['amount'];
            if ($calc_coupon) {
                $amount = $calc_coupon;
