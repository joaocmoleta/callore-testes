...
class PixPagarMeSend implements ShouldQueue
{
    ...
    public function handle()
    {
        if (env('PAGARME_URL') == '' || env('PAGARME_SK') == '') {
            Log::error(static::class . ' linha ' . __LINE__ . ' Erro nas envioments utilizadas no pagamento (PixPagarMe).');
            Log::info(static::class . ' linha ' . __LINE__ . ' PAGARME_DEBUG (PixPagarMe): ' . env('PAGARME_DEBUG'));
            Log::info(static::class . ' linha ' . __LINE__ . ' PAGARME_URL (PixPagarMe): ' . env('PAGARME_URL'));
            Log::info(static::class . ' linha ' . __LINE__ . ' PAGARME_SK (PixPagarMe): ' . env('PAGARME_SK'));
            Log::info(static::class . ' linha ' . __LINE__ . ' PAGARME_SK_PWD (PixPagarMe): ' . env('PAGARME_SK_PWD'));
            exit();
        }

        Log::info("Iniciado preparo dos dados para pagamento PIX PagarMe para ordem " . $this->other['order'] . '.');
        ...
        $fields = [
            "customer" => [
                ...
                "birthdate" => Carbon::create($order->birth)->format('d/m/Y')   // "01/01/1990"
            ],
            "items" => [],
            "payments" => [
                [
        ...
        Log::info("Payload (PixPagarMe): " . $fields);
        ...
        if ($err) {
            ...
            PixPagarMeProcess::dispatch([$response, $this->other['order']]);
        }

        Log::info("Finalizado envio de pagamento pix PagarMe para ordem " . $this->other['order'] . ". Com ou sem erros.");
    }
}
